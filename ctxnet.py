...
class CtxVGG(CtxNIN):
    def create_net(self, name, img_size, batchsize=32):
        ...
        shop.append(Conv2D('street-conv6', 128, 3, 2, cudnn_prefer='limited_workspace', workspace_byte_limit=1500, pad=0, input_sample_shape=shop[-1].get_output_sample_shape()))
        shop.append(Activation('street-conv6-relu', input_sample_shape=shop[-1].get_output_sample_shape()))
        shop.append(TagAttention('shop-tag',
            input_sample_shape=[shop[-1].get_output_sample_shape(), (self.ntag, )]))
        shop.append(L2Norm('shop-l2', input_sample_shape=shop[-1].get_output_sample_shape()))
        ...
        user.append(Conv2D('shop-conv6', 128, 3, 2, cudnn_prefer='limited_workspace', workspace_byte_limit=1500, pad=0, input_sample_shape=user[-1].get_output_sample_shape()))
        user.append(Activation('shop-conv6-relu', input_sample_shape=user[-1].get_output_sample_shape()))
        user.append(ContextAttention('street-cxt',
            input_sample_shape=[user[-1].get_output_sample_shape(), user[-1].get_output_sample_shape()]))
        user.append(L2Norm('street-l2', input_sample_shape=user[-1].get_output_sample_shape()))
